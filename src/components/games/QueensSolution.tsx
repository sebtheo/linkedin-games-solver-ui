...
const QueensSolution: React.FC<QueensSolutionProps> = ({
  solution,
  board,
  gridSize,
  isExpanded,
  ...
  const queenPositions = new Set(solution.map(([row, col]) => `${row},${col}`));

  return (
    <GameCard
      gameInfo={getGameInfo("queens")}
    ...
    >
      <div className="text-center">
            ...
            row.map((cellColor, colIndex) => {
              ...
              return (
                ...
                >
                  {isQueen && (
                    <span
                      className="text-xl text-slate-800 dark:text-slate-900"
                      aria-label="Queen"
                    >
                      ♛
                    </span>
        ...
        <div className="text-sm text-slate-600 dark:text-slate-400 mt-4">
          <p>
            Queens requires placing queens on a chess board so they don't
            threaten each other.
          </p>
        </div>
      </div>
