...
export const GAME_NAMES_LIST = GAME_SLUGS.map(
  (slug) => GAMES[slugToGameKey(slug)].name,
).join(", ");

export function slugToGameKey(slug: string): GameType {
  if (slug === "mini-sudoku") return "mini_sudoku";
  if (GAME_SLUGS.includes(slug as GameSlug)) {
    return slug as GameType;
  }
  throw new Error(`Unknown game slug: ${slug}`);
}

export function gameKeyToSlug(key: GameType): GameSlug {
  if (key === "mini_sudoku") return "mini-sudoku";
  return key as GameSlug;
}
...
export function getGameSeoTitle(gameSlug: GameSlug): string {
  const info = GAMES[slugToGameKey(gameSlug)];
  return `LinkedIn ${info.name}`;
}
